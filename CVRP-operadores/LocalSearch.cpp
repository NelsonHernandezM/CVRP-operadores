...
void busquedaLocalVND(Solution& sol, CVRP* problema, int bestImprovement, EstrategiaRuta est_ruta, const std::vector<std::vector<int>>& listaCandidatos) {
	...
	// --- CAMBIO CLAVE: Se crea e inicializa UNA SOLA VEZ aquí ---
...
void perturbarSolucionScramble(std::vector<std::vector<int>>& rutas, int num_clientes_total) {
	...
	for (size_t i = 0; i < rutas.size(); ++i) {
		if (tamanos_rutas[i] > 0) {
			// Asigna la porción correspondiente de clientes a la ruta
...
void LocalSearch::execute(Solution y) {
	...
	// 4. Bucle principal de Búsqueda Local Iterada (ILS) 
	while (iteraciones_sin_mejora < MAX_ITER_SIN_MEJORA) {
		...
		// Búsqueda local sobre la solución perturbada, usando la lista de candidatos 
	...
	// Calculamos la duración y la convertimos a milisegundos
	...
	//std::cout << "--------------------------------------------------" << std::endl;
	//std::cout << "Tiempo de ejecución de LocalSearch: " << seconds << " s" << std::endl;
	//std::cout << "--------------------------------------------------" << std::endl;
