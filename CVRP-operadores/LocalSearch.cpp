...
/**
 * @brief Convierte la representación de una solución en un vector de rutas de clientes.
 * @param sol La solución a decodificar.
 * @param problema Puntero al problema CVRP para identificar el depósito.
 * @return Un vector de vectores, donde cada vector interno es una ruta.
 */
...
void LocalSearch::execute(Solution y) {
	...
	while (iteraciones_sin_mejora < MAX_ITER_SIN_MEJORA) {
		...
		if (rnd->nextDouble() < 0.3) {
			perturbarSolucionScramble(rutas_candidatas, nodeLocations, problema->getNumberCustomers(), problema);
		}
		else {
			perturbarSolucionSwap(rutas_candidatas, nodeLocations, problema->getNumberCustomers(), problema);
		}
